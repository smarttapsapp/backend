    ...
    try:
        ...
        if user:
            ...
            if createdOTP:
                ...
                if authToken:
                    email_body = util.templates.TemplateResponse("otp.html",{"request": request, "user": user,"otp":createdOTP.otp},)
                    background_task.add_task(
                    util.mailer,
                    str(email_body.body, "utf-8"),
                    setting=setting,
        ...
        if account:
            ...
            if account.pin and util.verify_password(payload.pin,account.pin):
                ...
                if payload.action.lower() == "unlock":
                    ...
                    email_body = util.templates.TemplateResponse("otp.html",{"request": request, "user": account,"otp":createdOTP.otp},)
                    background_task.add_task(
