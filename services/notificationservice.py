...
from models.queries import notificationQuery
...
def getNotifications(db: Session,userId:int):
    notifications = notificationQuery.notifications(db=db,userId=userId)
    logger.info(notifications)
    return NotificationsResponse(statusCode="00",statusDescription=SUCCESS,data=notifications)
